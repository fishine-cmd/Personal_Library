...
2. 给文献附加作者、关键词、自定义标签、分类、出版社、期刊/会议来源、阅读状态、评分、个人笔记等信息。
...
7. `_persist_document_form()` 再调用 `upsert.py` 里的函数处理作者、关键词、标签、来源等
...
1. 在文献列表页可以使用基础搜索框，模糊匹配标题、摘要、作者、来源、关键词、标签和 DOI。
2. 展开“高级检索”后，可以按标题、摘要、作者、刊物/来源、关键词、标签、年份起止范围组合筛选。
3. 新增或编辑文献时，可以填写“标签”，例如 `化学, 高分子`。
4. 详情页和列表展开区的标签可以点击，直接按该标签回到列表检索。
...
1. `Document` 是系统中心。
2. 作者、关键词、标签、单位不是直接写死在 `Document` 一张表里，而是拆成独立表。
...
1. 扫描未被引用的字典项
2. 删除孤立作者、单位、来源、出版社、关键词、标签
3. 检测名称相似的潜在重复项
4. 文献删除后，顺带清理其周边孤立字典项和标签
...
4. 高级检索条件
5. 文献表格展示
6. 列表内展开，由关键信息查看摘要、关键词、标签、附件等信息
...
3. `list_documents()` 查询 `Document` 及相关作者、关键词、标签等信息
...
4. `_persist_document_form()` 处理：
   ...
   - 标签
   - 附件
5. 作者、关键词、标签、来源继续通过 `upsert.py` 查重或创建
6. 保存成功后跳转到详情页
...
2. 可传入基础搜索词 `q`
3. 可按分类、类型、单一年份筛选
4. 可在高级检索里按标题、摘要、作者、来源、关键词、标签、年份起止范围筛选
5. 后端构造 SQLAlchemy 查询，明确字段之间按 AND 组合
6. 基础搜索词 `q` 在标题、摘要、DOI、作者、来源、关键词和标签之间做 OR 模糊匹配
7. 结果按更新时间倒序显示，列表行可展开查看部分详情，再进入完整详情页
...
1. 页面展示当前用户所有作者、单位、来源、出版社、关键词、标签
2. 点击“检测冗余数据”
3. 后端扫描是否有未被文献使用的记录
4. 结果以 JSON 返回页面
5. 前端弹窗显示扫描结果
...
| `password_hash` | 攻击者只能离线暴力破解弱密码，强密码仍安全；不能伪造 session |
| `SECRET_KEY & password_hash` | 完全攻陷 |
